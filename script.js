...
function processDeposit() {
    ...
    // Calculate balance before deposit
    const totalDeposits = deposits.reduce((sum, d) => sum + d.amount, 0);
    const totalWithdrawals = withdrawals.reduce((sum, w) => sum + w.amount, 0);
    const totalPnL = trades.reduce((sum, t) => sum + t.pnl, 0);
    const balanceBefore = startingBalance + totalDeposits - totalWithdrawals + totalPnL;
    ...
    document.getElementById('newBalanceAfterDeposit').value = '';
...
function processWithdrawal() {
    ...
    // Calculate balance before withdrawal
    const totalDeposits = deposits.reduce((sum, d) => sum + d.amount, 0);
    const totalWithdrawals = withdrawals.reduce((sum, w) => sum + w.amount, 0);
    const totalPnL = trades.reduce((sum, t) => sum + t.pnl, 0);
    const balanceBefore = startingBalance + totalDeposits - totalWithdrawals + totalPnL;
...
function getEquityData(period) {
    ...
    if (period === '1m') {
        ...
        for (let d = new Date(startDate); d <= endDate; d.setDate(d.getDate() + 1)) {
            ...
            monthDates.set(dateStr, 0);
        }
        
        // Add actual changes
        sortedDates.forEach(date => {
            if (monthDates.has(date) && dailyChanges.has(date)) {
                monthDates.set(date, dailyChanges.get(date));
            }
        });
        ...
        // Sort month dates
        const monthDateArray = Array.from(monthDates.keys()).sort();
        
        labels.push('Start');
        data.push(runningBalance);
        
        monthDateArray.forEach(date => {
            runningBalance += monthDates.get(date) || 0;
            data.push(runningBalance);
        ...
        months.forEach(month => {
            runningBalance += monthlyData.get(month) || 0;
            data.push(runningBalance);
...
function setupEventListeners() {
    ...
    if (depositAmount) {
        depositAmount.addEventListener('input', function() {
            ...
            const totalWithdrawals = withdrawals.reduce((sum, w) => sum + w.amount, 0);
            const totalPnL = trades.reduce((sum, t) => sum + t.pnl, 0);
            const currentBalance = startingBalance + totalDeposits - totalWithdrawals + totalPnL;
            document.getElementById('newBalanceAfterDeposit').value = (currentBalance + amount).toFixed(2);
        });
    }
